...
class RelativelySmartSeeker(Agent):
    ...
    def act(
    ...
    ):  # TODO: make it so target doesn't continually update by re-adding self.target
        location = state.seeker_position
        curr_cell = self.map.find_cell(location)
        (
            self.cells_been_in.append(curr_cell)
            if curr_cell not in self.cells_been_in
            else None
        )

        while len(self.planned_cells) >= int(
            self.thirty_percent_of_cells / 3
        ):  # 10 percent of cells
        ...
        if self.hider_possible_loc is not None:  # update possible locations
            new_geom = self.map.polygon.intersection(
                self.hider_possible_loc.buffer(self.max_speed * 0.9)
            )
            if new_geom.geom_type in ["Point", "Polygon", "MultiPolygon"]:
                self.hider_possible_loc = new_geom  # type: ignore
            if (
                curr_cell is not None
                and self.hider_possible_loc is not None
        ...
        if (
            self.should_look_at_a_dead_end == 1 and self.dead_end_cell is not None
        ):  # currently going to dead end
            can_see = True
        ...
        if self.target == None or location == self.target:
            if self.should_look_at_a_dead_end >= 5 and len(self.dead_ends) > 5:
                new_target_cell = min(
                    self.dead_ends,
                    key=lambda cell: self.decide_dead_end(location, cell),
                )
        ...
        line_to_end = shapely.linestrings(
            [(location.x, location.y), (self.target.x, self.target.y)]
        )
    ...
    def decide_dead_end(self, location: shapely.Point, cell: NavMeshCell):
        distance = math.dist(
            (location.x, location.y), (cell.polygon.centroid.x, cell.polygon.centroid.y)
        )
        weight = 1
    ...
    def create_vector(self, state: WorldState) -> tuple[float, float]:
        ...
        return (
            (speed / distance) * (self.path[0].x - location.x),
            (speed / distance) * (self.path[0].y - location.y),
        )

    def signed_area(self, a, b, c):
        val = (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (
            c.x - a.x
    ...
    def funnel_path(self, target, state: WorldState) -> None:
        ...
        edges.append(
            [target, target]
        )  # this is a bootleg fix but adding the destination as a length-zero edge seems to help
    ...
    def astar(
        self, start_point: shapely.Point, end_point: shapely.Point, add_weight=True
    ) -> list[NavMeshCell] | None:
        # input points, output list of moves
        start_cell, end_cell = self.map.find_cell(start_point), self.map.find_cell(
            end_point
        )
        ...
        while frontier != []:
            ...
            for neighbor in curr.neighbors:
                ...
                if new_cost < g_scores[neighbor]:
                    heapq.heappush(
                        frontier,
                        (new_cost + neighbor.distance(end_cell), tie_break, neighbor),
                    )
